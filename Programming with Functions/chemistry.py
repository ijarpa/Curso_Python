...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for item in symbol_quantity_list:
        ...
        chemical_symbol = periodic_table_dict[symbol]

        mass = chemical_symbol[1]

        molar_mass += mass * quantity
...
def get_formula_name(formula, known_molecules_dict):
    ...
    keys = []
    items = []
    for key, item in known_molecules_dict.items():
        keys.append(key)
        items.append(item)
    ...
    if formula in keys:
        print(f"Name of chemical formula: {known_molecules_dict[formula]}")
    else:
...
def main():
    ...
    formula = get_formula_name(input("Enter a chemical formula:"), known_molecules_dict)

    # Get the mass of a chemical sample in grams from the user.
    mass = float(input("Enter the mass of the chemical: "))
    ...
    symbol_list = parse_formula(formula, periodic_table)
