...
def print_death_age(people_dict):
    ...
    for key, value in people_dict.items():
        age = (value[BIRTH_YEAR_INDEX] - value[DEATH_YEAR_INDEX]) * -1
        print(value[NAME_INDEX], age)
...
def count_genders(people_dict):
    ...
    for key, value in people_dict.items():
        ...
        for i in value[GENDER_INDEX]:
            if i == "F":
                count_F += 1
            elif i == "M":
                count_M += 1

    print(f"Count of males: {count_M}, count of females: {count_F}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_key, marriage_list in marriages_dict.items():
        husband_list = marriage_list[0]
        wife_list = marriage_list[1]
        year = marriage_list[2]

        husband_dict = people_dict[husband_list]
        husband_name = husband_dict[0]
        husband_age = (husband_dict[2] - marriage_list[2]) * -1

        wife_dict = people_dict[wife_list]
        wife_name = wife_dict[0]
        wife_age = (wife_dict[2] - marriage_list[2]) * -1

        wedding_year = marriage_list[2]

        print(husband_name, husband_age, wife_name, wife_age, wedding_year)
